...
async def get(ctx, tg_id: int):
    ...
    if params_value is None:
        await bot.send_message(
            tg_id,
            text=fmt.hitalic("Для получения результатов, укажите критерии поиска"),
        )
        ...
        if ads_av_kufar:
            for ad in ads_av_kufar:
                card = fmt.text(
                    fmt.text(
                        fmt.hbold("Марка и модель: "),
                        fmt.hlink((ad.brand + " " + ad.model), ad.link),
                    ),
                    fmt.text(fmt.hbold("Состояние: "), fmt.hitalic(ad.condition)),
                    fmt.text(fmt.hbold("Год: "), fmt.hitalic(ad.year)),
                    fmt.text(
                        fmt.hbold("Тип двигателя и объем: "),
                        fmt.hitalic(ad.engine_type, ad.engine_capacity),
                    ),
                    fmt.text(
                        fmt.hbold("Дата и место публикации: "),
                        fmt.hitalic(ad.date_time_ad, ad.region, ad.city),
                    ),
                    fmt.text(
                        fmt.hbold("Цена: "),
                        "Br",
                        fmt.hitalic(ad.price_br),
                        "Usd",
                        fmt.hitalic(ad.price_usd),
                    ),
                    sep="\n",
                )
            ...
            await bot.send_message(
                ...
                text=fmt.text(
                    ...
                    fmt.text(
                        fmt.hitalic("Для обновления параметров поиска"),
                        fmt.hbold(" /begin"),
                    ),
                    sep="\n\n",
                ),
