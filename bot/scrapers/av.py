...
class Av(DataClass):
    ...
    async def _get_date_ad(date: str) -> datetime:
        date_obj = datetime.datetime.strptime(
            date.replace("T", " ").split("+")[0], "%Y-%m-%d %H:%M:%S"
        ) + datetime.timedelta(hours=3)
        return date_obj

    @staticmethod
    async def _get_properties_ad(properties: list) -> dict:
        properties_car = dict.fromkeys(
            ("brand", "model", "year", "engine_capacity", "engine_type", "condition"),
            "Не указано",
        )
            ...
            async with session.get(url=page_url, headers=self.HEADERS) as response:
                ...
                for ad in ads["adverts"]:
                    ...
                    if date_ad > tracking_date:
                        ...
                        properties |= {
                            "region": ad["locationName"],
                            "city": ad["shortLocationName"],
                            "price_br": ad["price"]["byn"]["amount"],
                            "date_time_ad": date_ad,
                            "price_usd": ad["price"]["usd"]["amount"],
                            "link": ad["publicUrl"],
                            "tg_id": self.tg_id,
                        }
