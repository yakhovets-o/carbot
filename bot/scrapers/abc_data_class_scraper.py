...
class AbcDataClassScraper(ABC):
    ...
    def __post_init__(self):
        if self.__class__.__name__ == "Av":
            self.type_cars = {
                "cars" if self.cars else "truck",
                "truck" if self.truck_cars else "cars",
            }
            self.currency = "byn" if self.currency == "Br" else "usd"

            # query parameter url
            self.params = {
                f"price_{self.currency}[min]": self.price_min,
                f"price_{self.currency}[max]": self.price_max,
                "sort": 4,
            }
        ...
        if self.__class__.__name__ == "Kufar":
            ...
            self.type_cars = {
                2010 if self.cars else 2060,
                2060 if self.truck_cars else 2010,
            }
